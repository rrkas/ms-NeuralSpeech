...
def add_noise(token, op, candidate, unuse=None):
    if op == SUB:
        if candidate is None:
            ...
            prob_candidate = [
                i / sum(candidate_logit[: len(candidate)])
                for i in candidate_logit[: len(candidate)]
            ]
...
with open(infile, "r", encoding="utf-8") as infile:
    with open(outfile, "w", encoding="utf-8") as outfile:
        ...
        infile_lc = len(infile_lines)
        for count, line in enumerate(infile_lines):
            if count % 5000 == 1:
                print(
                    "{} / {} lines finished in {}s".format(
                        count - 1,
                        infile_lc,
            ...
            while i < sen_length:
                ...
                if random.random() < noise_ratio:
                    if tok not in sim_dict.keys():
                        ...
                        meta_new_token, meta_ins = noise_meta_beam(
                            tok, meta_noise, None
                        )
